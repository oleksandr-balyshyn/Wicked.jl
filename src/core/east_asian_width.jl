# Generated from Unicode 17.0.0 EastAsianWidth.txt (2025-07-24).
# Source: https://www.unicode.org/Public/UCD/latest/ucd/EastAsianWidth.txt
const _EAST_ASIAN_AMBIGUOUS_RANGES = Tuple{UInt32,UInt32}[
    (0x00A1, 0x00A1), (0x00A4, 0x00A4), (0x00A7, 0x00A8),
    (0x00AA, 0x00AA), (0x00AD, 0x00AE), (0x00B0, 0x00B4),
    (0x00B6, 0x00BA), (0x00BC, 0x00BE), (0x00BF, 0x00BF),
    (0x00C6, 0x00C6), (0x00D0, 0x00D0), (0x00D7, 0x00D8),
    (0x00DE, 0x00E1), (0x00E6, 0x00E6), (0x00E8, 0x00EA),
    (0x00EC, 0x00ED), (0x00F0, 0x00F0), (0x00F2, 0x00F3),
    (0x00F7, 0x00FA), (0x00FC, 0x00FC), (0x00FE, 0x00FE),
    (0x0101, 0x0101), (0x0111, 0x0111), (0x0113, 0x0113),
    ...
    (0x0300, 0x036F), (0x0391, 0x03A1), (0x03A3, 0x03A9),
    (0x03B1, 0x03C1), (0x03C3, 0x03C9), (0x0401, 0x0401),
    (0x0410, 0x044F), (0x0451, 0x0451), (0x2010, 0x2010),
    (0x2013, 0x2016), (0x2018, 0x201D), (0x2020, 0x2022),
    ...
    (0x2460, 0x249B), (0x249C, 0x24E9), (0x24EB, 0x24FF),
    ...
    (0x2642, 0x2642), (0x2660, 0x2665), (0x2667, 0x266D),
...
function _is_east_asian_ambiguous(character::Char)
    codepoint = UInt32(character)
    low = 1
    high = length(_EAST_ASIAN_AMBIGUOUS_RANGES)
    while low <= high
        middle = (low + high) >>> 1
        first_codepoint, last_codepoint = _EAST_ASIAN_AMBIGUOUS_RANGES[middle]
        if codepoint < first_codepoint
            high = middle - 1
        elseif codepoint > last_codepoint
            low = middle + 1
        else
            return true
        end
...
function _has_ambiguous_base(grapheme::AbstractString)
    for character in grapheme
        textwidth(character) == 0 && continue
        return _is_east_asian_ambiguous(character)
    end
    return false
end

function grapheme_width(policy::UnicodeWidthPolicy, grapheme::AbstractString)
    isempty(grapheme) && return 0
    width = clamp(textwidth(grapheme), 0, 2)
    policy.ambiguous_width == 2 && width == 1 && _has_ambiguous_base(grapheme) &&
        return 2
    return width
end
