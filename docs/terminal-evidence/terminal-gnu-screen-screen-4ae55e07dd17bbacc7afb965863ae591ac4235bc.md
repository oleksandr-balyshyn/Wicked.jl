...
| Terminal emulator, version, `TERM`, and `COLORTERM` | screen, TERM=screen-256color, COLORTERM=unset |
| Multiplexer and version | GNU screen 4.9 |
...
| Graphics either render through the negotiated protocol or fall back to Unicode | unicode fallback maintained under screen-style flow |
...
The GNU screen terminal evidence relies on PTY sequence restore checks and environment control-path validation from this commit.

## Risks and follow-up

Screen capture is represented by nested restore checks only.
