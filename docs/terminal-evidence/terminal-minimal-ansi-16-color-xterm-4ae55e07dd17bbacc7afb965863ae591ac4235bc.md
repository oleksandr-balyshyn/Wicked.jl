...
| Field | Value |
| --- | --- |
| Matrix category | Minimal ANSI / 16 color |
...
| Terminal emulator, version, `TERM`, and `COLORTERM` | xterm-16color, TERM=xterm, COLORTERM=unset |
...
| Graphics either render through the negotiated protocol or fall back to Unicode | Unicode fallback used |
...
The Minimal ANSI / 16 color terminal evidence relies on PTY sequence restore checks and environment control-path validation from this commit.

## Risks and follow-up

The 16-color path is representative and uses fallback validation.
